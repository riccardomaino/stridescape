...
class GetYearDistanceStatUseCaseTest{
    @get:Rule
    var instantTaskExecutorRule = InstantTaskExecutorRule()

    private lateinit var fakeWalkRepository: FakeWalkRepository
    private lateinit var getYearDistanceStatUseCase: GetYearDistanceStatUseCase

    @Before
    fun setUp() {
        fakeWalkRepository = FakeWalkRepository()
        getYearDistanceStatUseCase = GetYearDistanceStatUseCase(fakeWalkRepository)
    }

    @Test
    fun `get year distance with year range, should return the expected list of pairs`() {
        ...
        val result = getYearDistanceStatUseCase()
        assertThat(result).isEqualTo(actual)
    }

    @Test
    fun `get year distance with empty database, should return a list of pairs with zeros as the second element`() {
