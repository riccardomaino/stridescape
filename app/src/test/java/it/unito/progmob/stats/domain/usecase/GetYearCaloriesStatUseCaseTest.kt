...
class GetYearCaloriesStatUseCaseTest{
    @get:Rule
    var instantTaskExecutorRule = InstantTaskExecutorRule()

    private lateinit var fakeWalkRepository: FakeWalkRepository
    private lateinit var getYearCaloriesStatUseCase: GetYearCaloriesStatUseCase

    @Before
    fun setUp() {
        fakeWalkRepository = FakeWalkRepository()
        getYearCaloriesStatUseCase = GetYearCaloriesStatUseCase(fakeWalkRepository)
    }

    @Test
    fun `get year calories with year range, should return the expected list of pairs`() {
        ...
        val result = getYearCaloriesStatUseCase()
        assertThat(result).isEqualTo(actual)
    }

    @Test
    fun `get year calories with empty database, should return a list of pairs with zeros as the second element`() {
